...
;
	...
	void setContaminationClass(int c) {
		if(c < 0 || c > 10) throw new IllegalArgumentException();
		this.contClass = c;
	}
	...
	void advance(int currTime) {
		...
		while(this.estado == VehicleStatus.TRAVELING && i < currTime)
		{
			...
			this.localizacion = Math.min(localizacion + actSpeed, carretera/*carretera.lenght o algo asi*/);
			//Calcular la contaminacion:
			int I = this.localizacion - localizacionPrev;
			this.contaminacionT += this.contClass * I;
			//Comprobar si se sale de la carretera 
			if(this.localizacion >= this.road.lenght)
			{
