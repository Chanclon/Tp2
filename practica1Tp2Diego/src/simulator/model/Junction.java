...
public class Junction extends SimulatedObject implements LightSwitchingStrategy, DequeuingStrategy{
	...
	public void addIncommingRoad(Road r) {
		if(r.getDest() != this)
			throw new IllegalArgumentException("La carretera no pertenece l cruce");			this.roads.add(r);
	...
	void advance(int currTime) {
		if(currGreen != -1)
		{
			if(vehiclesInRoad.size() == 0) return;
			...
			lsStrategy.chooseNextGreen(roads, vehiclesInRoad, currGreen, prevGreen, currTime);
		}
	}
	...
	public JSONObject report() {
	    ...
	    jo.put("id", getId());
	    if(outRoads.size() == 0 && currGreen >= outRoads.size() - 1) jo.put("green", "none");
	    else jo.put("green", currGreen);
		...
		if(jo.isNull("queues") && jo.getJSONObject("queues").isEmpty()) jo.put("queues", "[]");
		else 
		{
			JSONArray queues = jo.getJSONArray("queues");
			
			JSONArray vehicles = jo.getJSONArray("vehicles");
			List<List<String>> v = new ArrayList<>();
			List<String> vId = new ArrayList<>();
			for(int j = 0; j < vehicles.length(); j++)
			{
				JSONObject vObject = queues.getJSONObject(j);
				vId.add(vObject.getString("vehicles"));
			}
			v.add(vId);
